...
// No default fallback - show "Not Available" if empty
const defaultStats = [];
...
export default function Home() {
    ...
    const stats =
        content.homeStats?.length > 0
            ? content.homeStats.map((stat) => {
                  ...
                  if (
                      stat.section_key === "budget-managed" &&
                      !automatedStats.loading
                  ) {
                      return {
    ...
    // Get dynamic content - no defaults
    const coreValues =
        content.coreValues?.length > 0
            ? content.coreValues.map((v) => v.title || v.content)
            : [];

    const aboutTitle = content.about?.title || "Not Available";
    const aboutContent =
                    ...
                    <motion.div
                        ...
                        {stats.map((stat) => (
                            <motion.div
                                ...
                                <p className="text-sm sm:text-base text-school-grey-600 mt-1">
                                    {stat.label}
                                </p>
                            </motion.div>
                        ))}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                        <motion.div
                            ...
                            <span className="text-university-red font-medium text-sm sm:text-base">
                                About USG
                            </span>
                            ...
                            {aboutContent
                                ...
                                .map((paragraph, index) => (
                                    <p
                                        key={index}
                                        className="text-school-grey-600 mb-4 sm:mb-6 text-sm sm:text-base">
                                        {paragraph}
                                    </p>
                                ))}
                            <Link
                                ...
                                <ul className="space-y-2">
                                    {coreValues.map((value, index) => (
                                        <motion.li
                                            ...
                                            <span className="text-school-grey-700 font-medium text-sm">
                                                {value}
                                            </span>
                                        </motion.li>
                                    ))}
                ...
                <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <motion.div
                        ...
                        <p className="text-base sm:text-lg text-white/80 mb-6 sm:mb-8 max-w-2xl mx-auto">
                            Your feedback shapes our initiatives. Share your
                            thoughts, concerns, or suggestions through TINIG
                            DINIG - our dedicated communication channel.
                        </p>
                        <Link
                            to="/feedback"
                            className="inline-block w-full sm:w-auto">
